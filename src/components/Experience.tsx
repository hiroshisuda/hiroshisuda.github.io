          ...
          <div className="space-y-12">
            {experience.map((entry, i) => (
              <div key={i} className="md:pl-10 relative">
                {/* Timeline dot */}
                <div className="absolute -left-[5px] top-2 w-2.5 h-2.5 rounded-full bg-accent-500 hidden md:block ring-4 ring-white dark:ring-slate-950" />
                ...
                <div className="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-6 hover:border-slate-300 dark:hover:border-slate-700 transition-colors">
                  <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-1 mb-4">
                    <div>
                      ...
                      <p className="text-accent-600 dark:text-accent-400 font-medium text-sm mt-0.5">{entry.company}</p>
                    </div>
                    ...
                    <ul className="space-y-2">
                      {entry.bullets.map((bullet, j) => (
                        <li key={j} className="flex gap-3 text-slate-600 dark:text-slate-400 text-sm leading-relaxed">
                          <span className="text-accent-500 leading-relaxed shrink-0">▸</span>
                          {bullet}
                        </li>
                      ))}
                    </ul>
...
function SectionHeading({ children }: { children: React.ReactNode }) {
  return (
    <h2 className="text-2xl font-bold text-slate-900 dark:text-slate-100 flex items-center gap-4">
      <span className="text-accent-500 dark:text-accent-400 font-mono text-xl">02.</span>
      {children}
